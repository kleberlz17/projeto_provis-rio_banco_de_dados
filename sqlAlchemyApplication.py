...
with Session(engine) as session:
    kleber = User(
        name = "Kleber Luiz",
        fullname = "Kleber Luiz Ferreira Machado",
        address = [Address(email_address= "kleberluizf15@gmail.com")]
    )

    bernardo = User(
        name='Bernardo Ferreira',
        fullname = "Bernardo Ferreira Machado Pontes",
        address = [Address(email_address= "bernardodk@gmail.com")]
                    
    )

    # enviando para o BANCO DE DADOS(persistência de dados)
    session.add_all([kleber, bernardo])

    session.commit()

stmt = select(User).where(User.name.in_(["kleber"]))
...
stmt_count = select(function.count('*')).select_from(User)
